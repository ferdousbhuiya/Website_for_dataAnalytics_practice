(function(){
  ...
  function bindQuiz(card,index){
    ...
    form.addEventListener('submit',e=>{
      ...
      out.textContent=score>=PASS_SCORE?`Passed: ${score}/5. The next checkpoint is now unlocked.`:`Score: ${score}/5. You need 4/5 to pass. Review Lessons ${index===0?1:CHECKPOINTS[index-1].end+1}–${cp.end} and retry.`;
      out.className='sql-quiz-result '+(score>=PASS_SCORE?'success':'warn');
      setTimeout(renderSqlFlow,650);
    });
  }
