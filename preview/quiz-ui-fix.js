(function(){
  ...
  function forceCurriculumInsideBeginner(){
    const topics=document.getElementById('topics');
    const cards=document.querySelectorAll('#path .path-grid .path-card');
    const beginner=cards[0];
    if(!topics||!beginner)return false;

    beginner.classList.add('curriculum-expanded-card','beginner-curriculum-host');
    if(topics.parentNode!==beginner){
  ...
  function install(){
    if(!document.getElementById('quizChoiceVisualStyles')){
      ...
      style.textContent=`
        ...
        #path .path-grid .path-card.beginner-curriculum-host{grid-column:1/-1!important;overflow:hidden!important;padding-bottom:0!important;border-color:#2f7650!important}
        #path .path-card.beginner-curriculum-host>#topics.curriculum-inside-stage{display:block!important;width:calc(100% + 2.2rem)!important;margin:1rem -1.1rem 0!important;padding:1rem 0 1.2rem!important;background:#fff!important;border-top:1px solid #e5ded4!important;border-radius:0!important;box-shadow:none!important}
        #path .path-card.beginner-curriculum-host>#topics.curriculum-inside-stage>.container{max-width:none!important;padding-left:1.1rem!important;padding-right:1.1rem!important}
        #path .path-card.beginner-curriculum-host>#topics .section-title{font-size:1.15rem!important}
        @media(max-width:760px){#path .path-card.beginner-curriculum-host>#topics.curriculum-inside-stage{width:calc(100% + 2.2rem)!important;margin:.9rem -1.1rem 0!important}}
      `;
      document.head.appendChild(style);
    }
    enhance();
    forceCurriculumInsideBeginner();
    setTimeout(forceCurriculumInsideBeginner,200);
    setTimeout(forceCurriculumInsideBeginner,800);
    setInterval(forceCurriculumInsideBeginner,2000);
