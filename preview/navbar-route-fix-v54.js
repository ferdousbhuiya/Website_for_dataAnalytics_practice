...
function hideModernLearning(){
  ...
  ['dpExcelShell','dpSubjectShell','learningView','aboutView','projectsView'].forEach(id=>{const el=document.getElementById(id);if(el)el.classList.add('hidden')});
...
function bind(){
  document.querySelectorAll('.navbar a[href^="#"]').forEach(a=>{
    ...
    if(!['home','path','calculator','projects','progress','about'].includes(id)||a.dataset.v55==='1')return;
    a.dataset.v55='1';
    a.addEventListener('click',e=>{
      e.preventDefault();e.stopImmediatePropagation();
      if(MAIN_IDS.includes(id)){go(id);active(id);return}
      hideModernLearning();
...
function styles(){if(document.getElementById('v55NavRouteStyles'))return;const s=document.createElement('style');s.id='v55NavRouteStyles';s.textContent='.navbar{pointer-events:auto!important}.navbar .nav-content,.navbar .nav-links,.navbar .nav-link,#dpAccountBtn{pointer-events:auto!important}';document.head.appendChild(s)}
