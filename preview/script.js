...
(function () {
  ...
  function injectLearnerStartBar() {
    ...
    bar.innerHTML = `
      <div class="learner-start-copy">
        ...
        <span>Start at the level that matches your current experience.</span>
      </div>
    ...
    bar.querySelectorAll('.start-level-btn').forEach(btn => {
      btn.addEventListener('click', () => setLearningLevel(btn.dataset.level));
    });
    ...
    if (preferred) {
      bar.querySelector(`[data-level="${preferred}"]`)?.classList.add('active');
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    cleanCurriculumSearch();
    setTimeout(injectLearnerStartBar, 200);
  }, { once: true });
