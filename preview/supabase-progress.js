...
.dp-account-btn{flex:0 0 auto;white-space:nowrap;margin-left:.7rem;border:1px solid rgba(126,218,171,.32);background:#0b1d29;color:#eaf6ef;border-radius:9px;padding:.5rem .78rem;font:700 .72rem/1 Inter,sans-serif;cursor:pointer;box-shadow:none;transition:background .2s ease,border-color .2s ease,color .2s ease,transform .2s ease}
.dp-account-btn:hover{background:#102936;border-color:#42df8c;color:#fff;transform:translateY(-1px)}
.dp-account-btn.signed{border-color:#2bcf7f;color:#7df0ad;background:#0b251c}
.navbar .nav-content{display:flex!important;align-items:center!important;flex-wrap:nowrap!important;gap:.45rem}
.navbar .brand{flex:0 0 auto}.navbar .nav-links{flex:1 1 auto;justify-content:center;min-width:0}.navbar .progress-cta{flex:0 0 auto}
...
.dp-auth-card{position:relative;width:min(430px,94vw);background:linear-gradient(180deg,#0a1b27,#081720);border:1px solid #2d5265;color:#eef8f2;border-radius:16px;padding:1.25rem 1.25rem 1.15rem;box-shadow:0 24px 80px rgba(0,0,0,.5)}
...
.dp-auth-card input{width:100%;box-sizing:border-box;margin:.32rem 0;padding:.78rem .72rem;border:1px solid #31566a;border-radius:8px;background:#0d2331;color:#fff;outline:none}.dp-auth-card input:focus{border-color:#42df8c;box-shadow:0 0 0 3px rgba(66,223,140,.1)}
.dp-auth-actions{display:flex;gap:.5rem;flex-wrap:wrap;margin-top:.72rem}.dp-auth-actions button{border:1px solid transparent;border-radius:8px;padding:.62rem .82rem;font-weight:800;cursor:pointer}
.dp-auth-primary{background:#39dc83;color:#052315}.dp-auth-primary:hover{background:#54e995}.dp-auth-secondary{background:#173141;color:#eaf6ef;border-color:#5d8297!important}.dp-auth-secondary:hover{background:#1d3c4e}.dp-auth-danger{background:#4a2024;color:#ffdadd}
.dp-auth-msg{display:none;min-height:1rem;margin:.65rem 0 0!important;padding:.55rem .65rem;border-radius:7px;background:#0c2730;color:#9de9bd!important;font-size:.72rem!important}.dp-auth-msg.show{display:block}.dp-auth-msg.error{background:#321d21;color:#ffc4c9!important}
.dp-auth-close{position:absolute;right:1rem;top:1rem;border:1px solid #506977!important;border-radius:6px!important;background:#102431!important;color:#b8c9d4!important;font-size:1rem!important;width:30px;height:30px;padding:0!important;line-height:1!important}.dp-auth-close:hover{border-color:#86a3b3!important;color:#fff!important}
.dp-auth-helper{font-size:.67rem;color:#7893a4;margin-top:.35rem}
...
`;document.head.appendChild(s)}
  ...
  function validateCredentials(d,msg){const email=d.querySelector('[data-email]').value.trim(),password=d.querySelector('[data-password]').value;if(!email){showMsg(msg,'Enter your email address.',true);return null}if(!/^\S+@\S+\.\S+$/.test(email)){showMsg(msg,'Enter a valid email address.',true);return null}if(!password){showMsg(msg,'Enter your password.',true);return null}if(password.length<6){showMsg(msg,'Password must contain at least 6 characters.',true);return null}return {email,password}}
  ...
  function openAuth(){
    ...
    d.innerHTML=user?`<div class="dp-auth-card"><button class="dp-auth-close" data-close aria-label="Close">×</button><h2>Learning account</h2><p>Signed in as <strong>${user.email||'learner'}</strong>. Your learning progress is synced securely across devices.</p><div class="dp-auth-actions"><button class="dp-auth-primary" data-sync>Sync now</button><button class="dp-auth-secondary" data-signout>Sign out</button></div><p class="dp-auth-msg"></p></div>`:`<div class="dp-auth-card"><button class="dp-auth-close" data-close aria-label="Close">×</button><h2>Save your progress</h2><p>Sign in or create an account to keep Beginner, Intermediate and Advanced progress across devices.</p><input data-email type="email" autocomplete="email" placeholder="Email"><input data-password type="password" autocomplete="current-password" minlength="6" placeholder="Password (minimum 6 characters)"><div class="dp-auth-helper">Your account is used only to save and restore your learning progress.</div><div class="dp-auth-actions"><button class="dp-auth-primary" data-signin>Sign in</button><button class="dp-auth-secondary" data-signup>Create account</button></div><p class="dp-auth-msg"></p></div>`;
    ...
    d.querySelector('[data-signin]')?.addEventListener('click',async()=>{const c=validateCredentials(d,msg);if(!c)return;showMsg(msg,'Signing in…');const {error}=await client.auth.signInWithPassword(c);showMsg(msg,error?friendlyError(error):'Signed in. Loading your progress…',!!error)});
    d.querySelector('[data-signup]')?.addEventListener('click',async()=>{const c=validateCredentials(d,msg);if(!c)return;showMsg(msg,'Creating account…');const {data,error}=await client.auth.signUp(c);showMsg(msg,error?friendlyError(error):(data.session?'Account created and signed in.':'Account created. Check your email to confirm your account.'),!!error)});
  }
