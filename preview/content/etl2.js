...
Check required columns, row count, duplicate business keys, missing critical fields, expected data types, and valid ranges.
...
Typical transformations include type conversion, text standardization, date parsing, code mapping, derived columns, reference-data joins, and aggregation.`
    },
    {
      number: 5,
...
Store rejected rows with the run date and reason so they can be traced and reprocessed.`
    },
    {
      number: 6,
      title: "Load Without Creating Duplicates",
      content: `A pipeline may run more than once. The load step should therefore avoid duplicate records.

Common approaches include append-only event loads, partition replacement, unique-key upserts, and stage-then-merge loading.
...
Capture pipeline name, run ID, start/end time, source rows, loaded rows, rejected rows, status, and failure details.
...
This information becomes the foundation for monitoring and troubleshooting.`
    },
    {
      number: 8,
      title: "Build a Small End-to-End ETL Function",
      content: `Combine the concepts into a small workflow.
...
**Checkpoint:** explain extract, validate, transform, reject, load, and audit stages before moving on.`
    }
  ],
  questions: [
    { number: 1, difficulty: "beginner", question: "Why divide an ETL program into stages?", answer: "So extraction, validation, transformation, loading and monitoring can be tested and maintained independently." },
    { number: 2, difficulty: "beginner", question: "Why preserve raw source files?", answer: "They provide an unchanged source of truth for auditing, debugging and reprocessing." },
    ...
    { number: 4, difficulty: "intermediate", question: "What is the purpose of rejected records?", answer: "They preserve failed rows with reasons so they can be investigated and reprocessed." },
    { number: 5, difficulty: "intermediate", question: "What does idempotent mean for ETL?", answer: "Rerunning safely does not create incorrect duplicates or inconsistent results." },
    { number: 6, difficulty: "intermediate", question: "Name three useful ETL audit fields.", answer: "Examples include run ID, start/end time, source rows, loaded rows, rejected rows, status and error message." }
  ],
  caseStudyQuizzes: [
    {
      case: 1,
      scenario: "A nightly orders file arrives with amount renamed to order_amount.",
